clear
path= 'E:\Photometry_FP0_FP3\Videos Barnes D2';
cd(path)
...
framerate  = 30;
maze_scale = [100/1018 100/1014;
                        100/1056 100/1050;
                        100/1029 100/1039]; % cm per pixel

cohort = [1*ones(1,12),2*ones(1,18),3*ones(1,16)];
...
for i = 1:length(dlc_results)
    ...
    p{i} = R_dlc{i}(:,17:18);   % body parts
    p_cm{i} = [p{i}(:,1)*maze_scale(cohort(i),1), p{i}(:,2)*maze_scale(cohort(i),2)]; % covert the location to cm
    ...
    for t = 2:length(p_cm{i})
        v{i}(t) = pdist2(p_cm{i}(t,:),p_cm{i}(t-1,:))*framerate;
