...
for i =1:length(files)
    if ismember(i,[pilo,ctrl])
        ...
        %     w = w+overlap;
        % end   % use this to generate mean_amp and mean_v
        ...
        subplot(5,1,[3 4 5])
        mdl = fitlm(mean_v{i}, mean_amp{i})
        [R{i},P{i}] = corrcoef(mean_v{i}, mean_amp{i});
        plot(mdl)
        ...
        savefig(gcf, [files(i).folder,'\v_fig_no_thresh.fig'])
        exportgraphics(gcf,[files(i).folder,'\v_fig_no_thresh.png'],'Resolution',300)
        save([files(i).folder,'\v_info_no_thresh',],"mdl")
        close
    end
end
amp_pool_all = cat(2,mean_amp{:});
v_pool_all = cat(2,mean_v{:});
amp_pool_pilo = cat(2,mean_amp{pilo});
v_pool_pilo = cat(2,mean_v{pilo});
amp_pool_ctrl = cat(2,mean_amp{ctrl});
...
xlim([min(cell2mat(mean_v),[],'all') max(cell2mat(mean_v),[],'all')])
ylim([min(cell2mat(mean_amp),[],'all') max(cell2mat(mean_amp),[],'all')])
savefig(gcf, [path,'\v_fig_all_animal_no_thresh.fig'])
exportgraphics(gcf,[path,'\v_fig_all_animal_no_thresh.png'],'Resolution',300)
save([path,'\v_info_all_animal_no_thresh'],'mdl_pool_all')
close
figure
set(gcf, 'Position', [100 100 900 900])
plot(mdl_pool_pilo)
...
xlim([min(cell2mat(mean_v),[],'all') max(cell2mat(mean_v),[],'all')])
ylim([min(cell2mat(mean_amp),[],'all') max(cell2mat(mean_amp),[],'all')])
savefig(gcf, [path,'\v_fig_all_pilo_no_thresh.fig'])
exportgraphics(gcf,[path,'\v_fig_all_pilo_no_thresh.png'],'Resolution',300)
save([path,'\v_info_all_pilo_no_thresh'],'mdl_pool_pilo')
close
figure
set(gcf, 'Position', [100 100 900 900])
plot(mdl_pool_ctrl)
...
xlim([min(cell2mat(mean_v),[],'all') max(cell2mat(mean_v),[],'all')])
ylim([min(cell2mat(mean_amp),[],'all') max(cell2mat(mean_amp),[],'all')])
savefig(gcf, [path,'\v_fig_all_ctrl_no_thresh.fig'])
exportgraphics(gcf,[path,'\v_fig_all_ctrl_no_thresh.png'],'Resolution',300)
save([path,'\v_info_all_ctrl_no_thresh'],'mdl_pool_ctrl')
close

% save([path,'\v_amp_windowed'],"mean_v","mean_amp")
save([path,'\Coef_v_amp_no_thresh'],"R","P","R_pool_pilo","R_pool_ctrl",...
